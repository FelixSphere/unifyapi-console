...
### Model Square 上的"新用户价"只显示你主动设过的那一条

首页和 Model Square 展示的 `default_group_model_ratio`，**只来自 `GroupModelDiscount` 里
`default` 组的逐模型覆盖**——也就是你主动为某个模型设过的那个数。没有设过的模型，对外一律
按**原价**展示。

`default` 组同时还有一个 `GroupRatio`（当前生产为 0.9），计费时**确实**会在没有逐模型覆盖
的情况下落到它身上。所以这里存在一个有意为之的不对称：

> **报价按原价，扣费可能更低；反过来绝不允许。**

不要把组倍率推导成"这个模型的对外折扣"。**运营规则，2026-09-22，就 `jev-1.13` 明确给出：
你没有主动设置折扣的模型，就是原价，不要替你推断。** 这条规则很容易被"修好"回去——
毕竟让橱窗和收银台一致听起来很对——所以它由
`controller/unifyapi_no_inferred_default_discount_test.go` 钉住，改回去会红。

推断的真正代价在于：**新进目录的模型天生没有覆盖**，一旦推断成立，每上一个新模型，
系统就自动替你承诺了一个你从没定过的价格。
