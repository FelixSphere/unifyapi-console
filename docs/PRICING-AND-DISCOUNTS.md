...
切换到官方报价基线会改动客户账单：`seed-model-discount-neutral.sql` 把每个模型的
折扣设成 `线上倍率 ÷ 官方倍率`，让这次切换在每张账单上都是 no-op。

不用它的话，按 24 天真实流量加权，**73.8% 的请求会涨价**——`claude-opus-4-8`
一个就占 54% 流量、涨 11.76 倍（它现在是官方价的 0.085，那是小数点错位，
不是商务决定）。

**价格中性不掩盖问题，反而把它仪表化。** 上线后对账按厂商真实价算成本，
第一份报表就会把 `claude-opus-4-8` 标成严重亏损并附上金额——那正是定真实价格
需要的数字。而且这一行存在之后，改价就只是后台改配置，可回退、不用发版。

用法：在同一个 SSM 会话里，**先**跑它，**再**跑 `seed-pricing.sql`
（前者对旧二进制无害，后者不是——原因见那个文件的头部）。

`setting/ratio_setting/unifyapi_price_neutral_test.go` 会对着一份生产快照断言
它确实中性，所以这不是注释里的承诺，是测出来的。
