	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			if err := service.SettleBilling(c, info, priceData.Quota); err != nil {
				common.SysLog("error settling Midjourney quota: " + err.Error())
			}
			...
			model.RecordConsumeLog(c, info.UserId, model.RecordConsumeLogParams{
				...
				ModelName: modelName,
				TokenName: tokenName,
				Quota:     service.CustomerChargeQuota(info, priceData.Quota, other),
				Content:   logContent,
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			model.RecordConsumeLog(c, relayInfo.UserId, model.RecordConsumeLogParams{
				...
				ModelName: modelName,
				TokenName: tokenName,
				Quota:     service.CustomerChargeQuota(relayInfo, priceData.Quota, other),
				Content:   logContent,
				TokenId:   relayInfo.TokenId,
				Group:     service.CustomerGroupForLog(relayInfo, other),
				Other:     other,
			})
