...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	pricingGroup := relayInfo.PricingGroup()
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		...
		ModelName:        logModel,
		TokenName:        tokenName,
		Quota:            CustomerChargeQuota(relayInfo, quota, other),
		Content:          logContent,
	...
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}}. Remaining balance: {{value}}. Please top up."
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else if notifyType == dto.NotifyTypeGotify {
				content = "{{value}}. Your remaining balance is {{value}}. Please top up."
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else {
				// 默认内容格式，适用于Email和Webhook（支持HTML）
				content = "{{value}}. Your remaining balance is {{value}}. Please top up to avoid interruption.<br/>Top up here: <a href='{{value}}'>{{value}}</a>"
