...
func TestGenerateOAuthCodeCarriesAffiliateInLoginFlow(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodPost, "/api/oauth/state", strings.NewReader(`{"provider":"auth-flow-test","intent":"login","aff":"invite-code","partnership":"PARTNER-EVENT"}`))
	...
	assert.Equal(t, "partner-event", payload.PartnershipCode)
