...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	// The currency the user was shown does not price the charge — Stripe does,
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe 创建 Checkout Session 失败 user_id=%d trade_no=%s amount=%d currency=%s error=%q", id, referenceId, req.Amount, shownCurrency.Code, err.Error()))
	...
	err = topUp.Insert()
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe 创建充值订单失败 user_id=%d trade_no=%s amount=%d currency=%s error=%q", id, referenceId, req.Amount, shownCurrency.Code, err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "创建订单失败"})
		return
	}
	// money is the stored credit basis, not a fiat amount. shown_currency is what
	// the wallet page estimated in; the currency actually charged is chosen by
	// Stripe Adaptive Pricing and arrives on the webhook as presentment_details.
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Stripe 充值订单创建成功 user_id=%d trade_no=%s amount=%d money=%.2f shown_currency=%s", id, referenceId, req.Amount, chargedMoney, shownCurrency.Code))
...
func genStripeLink(referenceId string, customerId string, email string, amount int64, successURL string, cancelURL string) (string, error) {
	...
	lineItem, err := stripeTopUpLineItem(amount)
	if err != nil {
		return "", err
	}
...
//
// Always bills the configured Price, in the account's settlement currency, with
// the credit count as the quantity — regardless of the currency the user was
// shown. Presenting a non-settlement currency ourselves would switch off Stripe
// Adaptive Pricing, which requires the session currency to be a settlement
// currency and skips any currency the price already defines. Adaptive Pricing
// localises the amount for buyers in 150+ markets at a rate Stripe guarantees
// through settlement and charges us nothing for, so handing that job to a rate
// this server stores would be strictly worse for both sides.
//
// The console's own currency selector is therefore an estimate for the wallet
// page only. Stripe decides the currency and the final amount at checkout.
func stripeTopUpLineItem(amount int64) (*stripe.CheckoutSessionLineItemParams, error) {
	// Checkout line items take a price, not a product; a prod_ id here would
	// otherwise fail as an opaque Stripe 400 on every topup attempt.
