...
// UNIFYAPI-FORK: the posted terms on which we take vendor credits.
//
// There is ONE deal. A seller hands us a key, we sell what it can serve, and
// they keep a posted share of what it sold for. Buying credits outright was
// withdrawn on 2026-09-22: it put our cash in front of a face value only the
// seller could see, against a key they could revoke, and nothing about it was
// ever offered to a seller anyway. See docs/credit-supply.md.
//
...
func ValidateCreditSupplyTerms(t CreditSupplyTerms) error {
	// An empty map is allowed: it is how an operator pauses intake entirely
	// without deleting the terms they will post again. Sellers then see no
	// vendors and the submission endpoint refuses every one.
