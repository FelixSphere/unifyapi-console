...
type CreditSupplyUsage struct {
	ChannelId int
	ModelName string
	// TokenUsage is the priced token breakdown, in the shape the catalogue
	// prices (cache reads and writes, and the upstream's usage semantic).
	ratio_setting.TokenUsage
...
func RecordCreditSupplyConsumption(usage CreditSupplyUsage) {
	...
	face, priced := ratio_setting.ListPriceUSD(modelName, usage.TokenUsage)
